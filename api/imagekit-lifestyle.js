export default async function handler(req, res) {
  ...
  // Map subcategories to folder names
  const folderMap = {
  ...
  if (!FOLDER) {
    return res.status(400).json({ error: 'Invalid subcategory. Must be: rockpooling, events, or traditional-wedding' });
  }
    ...
    const images = files
      ...
      .map(file => {
        ...
        return {
          ...
          alt: `Lifestyle ${subcategory} photo ${file.name}`
        };
      });
